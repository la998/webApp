...
class ModelMetaclass(type):
    ...
    def __new__(mcs, name, bases, attrs):
        ...
        logging.info('fund model: %s (table: %s)' % (name, table_name))
        mappings = dict()
        fields = []
        primarykey = None
        ...
        attrs['__select__'] = 'select `%s`,%s' % (primarykey, ', '.join(escaped_fields), table_name)
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    def get_value_or_default(self, item):
...
async def remove(self):
    ...
    if rows != 1:
        logging.warn('failed to remoave by primary key : affedted rows: %s' % rows)
